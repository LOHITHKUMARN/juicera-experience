...
const STRAWBERRY_IMAGE_CACHE: HTMLImageElement[] = [];

export const StrawberryBottleScroll: React.FC<StrawberryBottleScrollProps> = ({ progress }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  
  const [images, setImages] = useState<HTMLImageElement[]>(STRAWBERRY_IMAGE_CACHE);
  const [isMounted, setIsMounted] = useState(false);
  
  const FOLDER = "/images/ezgif-2ac927a590d0b067-jpg"; // Verified path
  const frameCount = 192;

  useEffect(() => {
    setIsMounted(true);
    if (STRAWBERRY_IMAGE_CACHE.length === frameCount) return;
    ...
    const load = async () => {
      ...
      if (!isCancelled && batch.length === frameCount) {
        STRAWBERRY_IMAGE_CACHE.length = 0;
        batch.forEach(img => STRAWBERRY_IMAGE_CACHE.push(img));
      }
    };
