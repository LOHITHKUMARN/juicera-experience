...
const MANGO_IMAGE_CACHE: HTMLImageElement[] = [];

export const MangoBottleScroll: React.FC<MangoBottleScrollProps> = ({ progress }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  
  const [images, setImages] = useState<HTMLImageElement[]>(MANGO_IMAGE_CACHE);
  const [isMounted, setIsMounted] = useState(false);
  
  const FOLDER = "/images/ezgif-5127e5d4f52c97c9-jpg"; // Verified path
  const frameCount = 192;

  useEffect(() => {
    setIsMounted(true);
    if (MANGO_IMAGE_CACHE.length === frameCount) return;
    ...
    const load = async () => {
      ...
      if (!isCancelled && batch.length === frameCount) {
        MANGO_IMAGE_CACHE.length = 0;
        batch.forEach(img => MANGO_IMAGE_CACHE.push(img));
      }
    };
